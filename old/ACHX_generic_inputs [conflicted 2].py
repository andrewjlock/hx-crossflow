...
# This is the main function that executes a result
    d3 = [0];v=[0]
    TC_out_ave, dp_amb_total, T1, Q_PF, PF_dp, TH_final, dp_amb, alpha_i_final, TW_H_final, TW_C_final, Q3, PH, TC_final, alpha_o_final, dT, mdot_vec, G, d3[0], v[0], Bu_c, Bu_j, T_out, UA = ACHX.main(ACHX_inputs,T0i = T0,fig_switch=0, mod=mod,verbosity=verbosity)
...
def ACHX_inputs(G,F,M,CT):    
    ...
    F.PF_fluid  = 'CO2'       
    F.Amb_fluid  = 'Air'
    F.T_PF_in  = 273.15 + 81.4 #  CO2 inlet temperature (K)
    ...
    F.TC_outlet_guess = 273.15+45.1 # Air side outlet guess for initialisation (K)
