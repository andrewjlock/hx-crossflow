...
# mdot_array = np.array([0.5])*344.4*0.643
Tin_array = np.array([70, 80, 100, 120])+273.15
pin_array = np.array([7.8, 8, 8.5, 9, 9.5, 10, 11])*1e6
# pin_array = np.array([11])*1e6
# d3 = 49.19553

P_array = np.linspace(7.8,20,30)*1e6
...
def run_model(indexes):
    ...
    def mod(A):
        ...
        if hasattr(A, 'TC_outlet_guess'):
            A.TC_outlet_guess = Tamb_array[i]+10
        if hasattr(A, 'T_PF_out'):
            A.T_PF_out = Tamb_array[i]+2
    # mod,indexes = args
...
CTdata = np.load("NDDCT_case-20_data_V4.npy")
# np.zeros((len(Tamb_array),len(mdot_array),len(Tin_array),len(pin_array)))
...
def perf_table():
    ...
    np.save("NDDCT_case-20_points_11MPa",CTpoints)
    old_data = np.load("NDDCT_case-20_data_V4.npy")
