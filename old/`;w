...
def main(ACHX_inputs,mod=None,v=None,TC_in=None,T0i=[0],HX_L=None,fig_switch=0,verbosity=0):
   ...
   # Creates initial solution values from inputs or interpolation function
    ...
    error, TH_final, TC_final, TW_H_final, TW_C_final, Q1, Q2, Q3, Q4, Q5, Q6, Q7, Q8, Q9, alpha_i_final, PH, dp_amb, mdot_vec,P_headers,v, alpha_o_final, dT, d3, Bu_c, Bu_j, Bu_p, UA = equations(T,G,F,M,I,1,CT)
    P_out = P_headers[-1]
    E_out = []
    if G.n_passes%2==0:
    ...
    return TC_out_ave, dp_amb_total, T, Q_PF, PF_dp, TH_final, dp_amb, alpha_i_final, TW_H_final, TW_C_final, Q3, PH, TC_final, alpha_o_final, dT, mdot_vec, G, d3, v, Bu_c, Bu_j, Bu_p, T_out, UA
...
def equations(T,G,F,M,I,flag,CT):
    ...
    if flag == 1:
        ...
        Q5 = []; Q6 = []; Q7 = []; Q8 = []; Q9 = []; 
        Alpha_i=[]; Alpha_o = []; dT = [] 
        Bu_c_array = []; Bu_j_array = []; Bu_p_array = []
        UA = []
    ...
    if M.solver_type == 1 or 2: # Header pressure for mdot or outlet temperature boundary condition [(n_passes)*(n_CP-1) equations]
        for d in range(G.n_passes): 
            ...
            for e in range(G.n_CP-1):
                ...
                error[4*G.n_cells + (G.n_passes-1)+ ((d*(G.n_CP-1))+e)] = np.sign(pc_delta_header_P)*(abs(pc_delta_header_P))*1e3
    ...
    if M.solver_type == 2: # Temperature outlet boundary condition
        ...
        # error[(4*G.n_cells) + (G.n_passes-1)+ ((G.n_passes)*(G.n_CP-1))] = (h_ave - h_design_out)*(sum(mdot_vec)/G.n_passes)*1e-1 #
        error[(4*G.n_cells) + (G.n_passes-1)+ ((G.n_passes)*(G.n_CP-1))] = (T_out - F.T_PF_out)*(sum(mdot_vec)/G.n_passes)*1e3 #
    ...
    for i in range(G.n_cells): 
        ...
        alpha_o = calc_alpha_amb(G,F,I,Pr=Pr_amb,Tb=Tb_amb,rho_b=rho_amb,Correlation=M.alpha_CorrelationC,K_c=K_c[row],row_correction=M.row_correction)
    ...
    if flag==0:
        ...
        return error, TH, TC, TW_H, TW_C, Q1, Q2, Q3, Q4, Q5, Q6, Q7, Q8, Q9, Alpha_i, PH, dp_amb, mdot_vec,P_headers, v, Alpha_o, dT, d3, Bu_c_array, Bu_j_array, Bu_p_array, UA
