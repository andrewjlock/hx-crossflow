...
def ACHX_inputs(G,F,M):    
    ...
    G.dx_i = 0.2501  #0.4 # (m) Initial length of pipe discretisation
    ...
    F.T_PF_out = 273.15+32 # For temperature boundary condition
...
dRTD = 0.02 # Accuracy of RTD (degC)
dmdot = 0.001 # Accuracy of Coriolis flow meter 0.1% for liquids
mdot = 0.2 # Mass flow rate used in modelling
...
datestamp = datetime.datetime.now().strftime("%I-%M-%S%p_%B_%d_%Y")
Filename = "Uncertainty_Results_"+datestamp+".csv"
