...
def main(ACHX_inputs,mod=None,v=None,TC_in=None,T0i=[0],HX_L=None,fig_switch=0,verbosity=1):
    start = time.time()
   ...
   # Creates initial solution values from inputs or interpolation function
    ...
    return TC_out_ave, dp_amb_total, T, Q_PF, PF_dp, TH_final, dp_amb, alpha_i_final, TW_H_final, TW_C_final, Q3, PH, TC_final, alpha_o_final, dT, mdot_vec, G, F, Bu_c, Bu_j, T_out
    ...
    if M.solver_type == 1 or 2: # Header pressure for mdot or outlet temperature boundary condition [(n_passes)*(n_CP-1) equations]
        for d in range(G.n_passes): 
            ...
            for e in range(G.n_CP-1):
                ...
                error[4*G.n_cells + (G.n_passes-1)+ ((d*(G.n_CP-1))+e)] = np.sign(pc_delta_header_P)*(abs(pc_delta_header_P))*1e3 # Error for similar pressure between parallel streams at headers
