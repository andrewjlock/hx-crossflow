...
def main(ACHX_inputs,mod=None,v=None,mdot=None,TC_in=None,T0i=[0],HX_L=None,fig_switch=0):
   ...
   # Creates initial solution values from inputs or interpolation function
    ...
    TH_final, TC_final, TW_H_final, TW_C_final = open_T(T,F.T_PF_in,F.TC_in,G.n_cells,G.n_rows,G.n_CPR)
...
def get_T0_iter(G,F,M,I):
    ...
    try: 
        ...
        T0[2*G.n_cells:3*G.n_cells] =  T0[0:G.n_cells] - ((T0[0:G.n_cells]-T0[G.n_cells:2*G.n_cells])*(alpha_o/3000)*(n_f*G.A_ft/G.dA_i))
        T0[3*G.n_cells:4*G.n_cells] =  T0[0:G.n_cells] - ((T0[0:G.n_cells]-T0[G.n_cells:2*G.n_cells])*(alpha_o/2500)*(n_f*G.A_ft/G.dA_i))
        ...
        T0[3*G.n_cells:4*G.n_cells] =  T0[0:G.n_cells]
        return T0
...
def equations(T,G,F,M,I,flag):
    ...
    error = np.zeros(4*G.n_cells)
    Q1 = []; Q2 = []; Q3 = []; Q4 = [] 
    Q5 = []; Q6 = []; Q7 = []; Q8 = []; Q9 = []; Alpha_i=[] 
    PH = [];
    PH.append(F.P_PF_in)

    TH,TC,TW_H,TW_C = open_T(T,F.T_PF_in,F.TC_in,G.n_cells,G.n_rows,G.n_CPR)
    ...
    for i in range(G.n_cells): 
        ...
        k_PF = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,PH[i],(0.5*(TH[i]+TH[i+1]))],[CPCP.iconductivity]) 
        km_PF = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,PH[i],TH[i]],[CPCP.iconductivity])  
        kp_PF = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,PH[i],TH[i+1]],[CPCP.iconductivity])  
        ...
        rho_PF_w = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,PH[i],TW_H[i]],[CPCP.iDmass]) 
        ...
        Nu_PF = calc_Nu(G,F,Re=Re_PF,Pr=Pr_PF,P=PH[i],Tb=Tb_PF,Tw=TW_H[i],rho_b=rho_PF_b,rho_w=rho_PF_w,Correlation=M.Nu_CorrelationH,mu_b = mu_PF)
        ...
        f_PF = calc_f(Re_PF,PH[i],TH[i],TH[i+1],TW_H[i],F.mdot_PF,G.A_CS,G.ID,F.PF_fluid,M.f_CorrelationH,q_pre/G.dA_i,rho_PF_b,rho_PF_w,mu_PF)
        ...
        q3 = -(TW_H[i] - ((TH[i]+TH[i+1])/2))*alpha_i*G.dA_i
        q_pre = q3
        ...
        q1_conv = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,PH[i],TH[i]],[CPCP.iHmass]) * F.mdot_PF 
        q2_conv = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,PH[i+1],TH[i+1]],[CPCP.iHmass]) * F.mdot_PF 
