...
def variable_inputs(G,F,M):
    ...
    G.pitch_transverse =  0.058   # - (m) Transverse tube pitch
    G.ID = 0.0216   # (m) ID of HX pipe
    G.t_wall = 0.001 # (m) Thickness of pipe wall
    G.dx_i = 1 # (m) Initial length of pipe discretisation
    ...
    F.fluidH  = 'CO2'    	
    F.fluidC  = 'Air'
    F.TH_in  = 273.15+71.1 # (K)
    F.mdot_H  =  158.9/(25*20) # (kg/s)
    F.P_PF_in  = 7.96*10**6 # pressure (Pa)
    F.TC_in  = 273.15+20 # (K)
    F.vC_in  = 1 # (m/s)
...
def main():
    ...
    TH_final, TC_final, TW_final = open_T(T,F.TH_in,F.TC_in,G.n_cells,G.n_rows,G.n_CPR)
    ...
    Q_PF = F.mdot_H * (CP('H','T',TH_final[0],'P',PH[0],F.fluidH) - CP('H','T',TH_final[G.n_cells],'P',PH[G.n_cells],F.fluidH))
    dE_PF = Q_PF
    q_amb_out = 0
    ...
    for i in range(G.n_CPR):
        qi = CP('H','T',TC_final[i],'P',P_amb_out,F.fluidC)
        q_amb_out = q_amb_out + qi
    Q_amb = (q_amb_out - (G.n_CPR*CP('H','T',F.TC_in,'P',F.P_amb_in,F.fluidC)))*F.mdot_C
    deltaQ = Q_amb-dE_PF
    ...
    return TH_final, TC_final, TW_final, Q1, Q2, Q3, Q4, Q5, Q6, Q7, Q8, alpha_i_final, PH, dp_amb
...
class Geometry:
    ...
    def micro_init(self):
        ...
        self.n_phi = ((self.D_fin/self.OD)-1)*(1+(0.35*(np.log(self.D_fin/self.OD))))
        if self.D_fin:
            self.A_f = (self.dx/self.pitch_fin)*((((self.D_fin**2)-(self.OD**2))*np.pi/2)+self.D_fin*self.t_fin)
            self.A_r = self.dA_o - ((self.dx/self.pitch_fin)*self.t_fin*self.OD*np.pi)
            self.A_ft = self.A_f + self.A_r
            self.A_FTCS = (self.OD*self.dx) + ((self.dx/self.pitch_fin)*self.t_fin*(self.D_fin-self.OD))
            self.A_c = self.A_amb - self.A_FTCS
            self.H_f = 0.5*(self.D_fin-self.OD)
            self.A_rat = (self.A_f + self.A_r)/self.A_r
            self.A_min = self.A_amb-self.A_FTCS
...
class Fluid:
    ...
    def micro_init(self,G):
        ...
        self.mdot_C = self.Vdot_C * CP('D','P',self.P_amb_in,'T',self.TC_in,self.fluidC)
        self.mdot_amb_max = self.mdot_C/G.A_min 
...
def get_T0_iter(G,F,M):
    ...
    try: 
        h_amb_in =  CP('H','T',F.TC_in,'P',F.P_amb_in,F.fluidC)
        h_PF_in =  CP('H','T',F.TH_in,'P',F.P_PF_in,F.fluidH)
        E_amb_in = h_amb_in * F.mdot_C * G.n_CPR 
        E_PF_in = h_PF_in * F.mdot_H
        TC_out_guess = F.TC_outlet_guess
        error = 1
        j = 0
        while error > 0.01:
            j = j+1
            h_amb_out =  CP('H','T',TC_out_guess,'P',F.P_amb_in,F.fluidC)
            dh_amb = (h_amb_out-h_amb_in)
            dh_PF = dh_amb*F.mdot_C*G.n_CPR/F.mdot_H
            h_PF_out = h_PF_in - dh_PF
            T_PF_out = CP('T','H',h_PF_out,'P',F.P_PF_in,F.fluidH) # PF temp out from enthalpy balance
            T_PF_av = ((2*T_PF_out) + F.TH_in)/3
            T_amb_av = 0.5*(F.TC_in+TC_out_guess) # bulk temperature
            Pr_amb = CP('PRANDTL','P',F.P_amb_in,'T', T_amb_av ,F.fluidC)
            rho_amb = CP('DMASS','P',F.P_amb_in,'T', T_amb_av ,F.fluidC)
            mu_amb = CP('VISCOSITY','P',F.P_amb_in,'T', T_amb_av ,F.fluidC)
            ...
            T_PF_out1 = CP('T','H',h_PF_in - (q/F.mdot_H),'P',F.P_PF_in,F.fluidH) # PF temp out from convective heat trasnfer balance
        ...
        T0[0:G.n_cells] = F.TH_in - (np.arange(G.n_cells)+1)/float(G.n_cells)*(F.TH_in - T_PF_out) # Assumes temperature does not drop below critical temperature
        ...
        T0 = np.zeros(3*G.n_cells) 
        T0[0:G.n_cells] = F.TH_in - (np.arange(G.n_cells)+1)/float(G.n_cells)*(F.TH_in - F.TC_outlet_guess)
        for i in range(G.n_rows):
...
def open_T(T,TH_in,TC_in,n_cells,n_rows,n_CPR):
    """
...
def calc_Nu(G,F,Re=0,Pr=0,P=0,Tb=0,Tw=0,rho_b=0,rho_w=0,Correlation=0,K_c=0):
    ...
    if Correlation == 2:  # Yoon et al corelation that incorporates wall temperature
        CP_b = CP('C','P',P,'T',Tb,F.fluidH)
        CP_w = CP('C','P',P,'T',Tw,F.fluidH)
...
def calc_alpha_amb(G,F,Re=0,Pr=0,Tb=0,rho_b=0,Correlation=0,K_c=0):
    ...
    mu = CP('VISCOSITY','P',F.P_amb_in,'T', Tb ,F.fluidC)
    k = CP('CONDUCTIVITY','P',F.P_amb_in,'T', Tb ,F.fluidC)
    ...
    if Correlation == 3: # From ASPEN HTFS3
        cp = CP('C','T',Tb,'P',F.P_amb_in,F.fluidC)
        U = F.mdot_C/(rho_b*G.A_amb)
        U_f = K_c * U
        Re_f = rho_b * U_f * G.OD/mu
        Re_max = F.mdot_amb_max * G.OD / mu
        j = 0.205 * (Re_f**0.04) * (Re_max**-0.368)*(G.A_rat**-0.15)
...
def calc_air_dp(G,n_rows,rho,mu):
    ...
    GR_eff = (G_D + G_A)/G_T
    theta = np.arctan(0.5*G.pitch_transverse/G.pitch_longitudal)*(np.pi/180)
...
def equations(T,G,F,M,flag):
    ...
    TH,TC,TW = open_T(T,F.TH_in,F.TC_in,G.n_cells,G.n_rows,G.n_CPR)
    ...
    for k in range(G.n_rows):
        rho_k = 0.25*(CP('D','T',TC[k*G.n_CPR],'P',F.P_amb_in,F.fluidC) + CP('D','T',TC[((k+1)*G.n_CPR)-1],'P',F.P_amb_in,F.fluidC) + \
                CP('D','T',TC[(k+1)*G.n_CPR],'P',F.P_amb_in,F.fluidC) + CP('D','T',TC[((k+2)*G.n_CPR)-1],'P',F.P_amb_in,F.fluidC))
        mu_k = 0.25*(CP('VISCOSITY','T',TC[k*G.n_CPR],'P',F.P_amb_in,F.fluidC) + CP('VISCOSITY','T',TC[((k+1)*G.n_CPR)-1],'P',F.P_amb_in,F.fluidC) + \
                CP('VISCOSITY','T',TC[(k+1)*G.n_CPR],'P',F.P_amb_in,F.fluidC) + CP('VISCOSITY','T',TC[((k+2)*G.n_CPR)-1],'P',F.P_amb_in,F.fluidC))
        U_k = F.mdot_C/(rho_k * G.A_amb)
        K_B,K_ft = calc_air_dp(G,1,rho_k,mu_k)
        K_c.append((K_B/K_ft)**(1/1.7))
        dp_amb.append(1.081 * K_B*rho_k*(U_k**1.7))
        print(dp_amb)
    for i in range(G.n_cells): 
        ...
        k_PF = CP('CONDUCTIVITY','P', PH[i], 'T', (0.5*(TH[i]+TH[i+1])) ,F.fluidH)
        km_PF = CP('CONDUCTIVITY','P', PH[i],                'T', TH[i] ,F.fluidH)
        kp_PF = CP('CONDUCTIVITY','P', PH[i],               'T', TH[i+1] ,F.fluidH)
        k_amb = CP('CONDUCTIVITY','P', F.P_amb_in, 'T', (0.5*(TC[i]+TC[i+1])) ,F.fluidC)
        km_amb = CP('CONDUCTIVITY','P', F.P_amb_in,                'T', TC[i] ,F.fluidC)
        kp_amb = CP('CONDUCTIVITY','P', F.P_amb_in,              'T', TC[i+1] ,F.fluidC)

        Tb_PF = 0.5*(TH[i]+TH[i+1]) # bulk temperature
        Tb_amb = 0.5*(TC[node_amb_i1]+TC[i]) # bulk temperature
        Pr_PF = CP('PRANDTL','P',PH[i],'T', Tb_PF ,F.fluidH)
        Pr_amb = CP('PRANDTL','P',F.P_amb_in,'T', Tb_amb ,F.fluidC)
        rho_PF_b = CP('DMASS','P',PH[i],'T', Tb_PF ,F.fluidH)
        rho_PF_w = CP('DMASS','P',PH[i],'T', TW[i] ,F.fluidH)
        rho_amb = CP('DMASS','P',F.P_amb_in,'T', Tb_amb ,F.fluidC)
        U_PF = abs(F.mdot_H / (rho_PF_b * G.A_CS))
        mu_PF = CP('VISCOSITY','P',PH[i],'T', Tb_PF,F.fluidH)    
        ...
        n_f = np.tanh(b*G.OD*G.n_phi)/(b*G.OD*G.n_phi)
        if i == 0:
            f_PF = calc_f(Re_PF,PH[i],TH[i],TH[i+1],TW[i],F.mdot_H,G.A_CS,G.ID,F.fluidH,M.f_CorrelationH,0,rho_PF_b,rho_PF_w,mu_PF)
        else:
            f_PF = calc_f(Re_PF,PH[i],TH[i],TH[i+1],TW[i],F.mdot_H,G.A_CS,G.ID,F.fluidH,M.f_CorrelationH,abs((TH[i-1] - TH[i]))/G.dA_i,rho_PF_b,rho_PF_w,mu_PF)
        ...
        q4 = -(((TC[node_amb_i1]+TC[i])/2) - TW[i])*alpha_o*G.A_ft*n_f
        
        # PF side bulk convection
        q1_conv = CP('HMASS','P',PH[i],'T',TH[i],F.fluidH) * F.mdot_H
        q2_conv = CP('HMASS','P',PH[i+1],'T',TH[i+1],F.fluidH) * F.mdot_H

        #   q1_cond=0;q2_cond=0;q5_cond=0;q6_cond=0;
        ...
        q5_conv = CP('HMASS','P',F.P_amb_in,'T',TC[node_amb_i1],F.fluidC) * F.mdot_C
        q6_conv = CP('HMASS','P',F.P_amb_in,'T',TC[i],F.fluidC) * F.mdot_C
...
TH_final, TC_final, TW_final, Q1, Q2, Q3, Q4, Q5, Q6, Q7, Q8, alpha_i_final, PH, dp_amb = main()
