...
def main(ACHX_inputs,mod=None,v=None,mdot=None,TC_in=None,T0i=[0],HX_L=None,fig_switch=0):
   ...
   # Creates initial solution values from inputs or interpolation function
    ...
    TH_final, TC_final, TW_H_final, TW_C_final,DV = open_T(T,F.T_PF_in,F.TC_in,G.n_cells,G.n_rows,G.n_CPR,G.n_CP,G.n_passes,F.P_PF_in,F.PF_fluid)
    h_c = []
    ...
    dE_PF = F.mdot_PF * G.n_CP * (CP('H','T',TH_final[0],'P',PH[0],F.PF_fluid) - CP('H','T',T_out,'P',PH[G.n_cells],F.PF_fluid))
...
def open_T(T,T_PF_in,TC_in,n_cells,n_rows,n_CPR,n_CP,n_passes,P_PF_in,PF_fluid):
    ...
    return TH,TC,TW_H,TW_C,dir_vec
...
def equations(T,G,F,M,I,flag):
    ...
    TH,TC,TW_H,TW_C,DV = open_T(T,F.T_PF_in,F.TC_in,G.n_cells,G.n_rows,G.n_CPR,G.n_CP,G.n_passes,F.P_PF_in,F.PF_fluid)
    ...
    for i in range(G.n_cells): 
        ...
        f_PF = calc_f(Re_PF,PH[i],TH[i+row],TH[i+1+row],TW_H[i],F.mdot_PF,G.A_CS,G.ID,F.PF_fluid,M.f_CorrelationH,q_pre/G.dA_i,rho_PF_b,rho_PF_w,mu_PF)
        ...
        if DV[row]>0:
            ...
            q2_conv = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,PH[i+1],TH[i+row+1]],[CPCP.iHmass]) * F.mdot_PF * DV[row] * -1
            ...
            q2_conv = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,PH[i],TH[i+row]],[CPCP.iHmass]) * F.mdot_PF * DV[row]
            q1_conv = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,PH[i+1],TH[i+row+1]],[CPCP.iHmass]) * F.mdot_PF * DV[row] *-1
