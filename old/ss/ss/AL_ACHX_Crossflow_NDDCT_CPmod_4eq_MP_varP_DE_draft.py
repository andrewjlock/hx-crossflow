...
def main(ACHX_inputs,mod=None,v=None,TC_in=None,T0i=[0],HX_L=None,fig_switch=0):
   ...
   # Creates initial solution values from inputs or interpolation function
    ...
    error, TH_final, TC_final, TW_H_final, TW_C_final, Q1, Q2, Q3, Q4, Q5, Q6, Q7, Q8, Q9, alpha_i_final, PH, dp_amb, mdot_vec,P_headers,v, alpha_o_final, dT, d3, Bu_c, Bu_j = equations(T,G,F,M,I,1,CT)
    P_out = P_headers[-1]
    E_out = []
    if G.n_passes%2==0:
    ...
    return TC_out_ave, dp_amb_total, T, Q_PF, PF_dp, TH_final, dp_amb, alpha_i_final, TW_H_final, TW_C_final, Q3, PH, TC_final, alpha_o_final, dT, mdot_vec, G, d3, v, Bu_c, Bu_j
...
def get_T0_iter(G,F,M,I,CT):
    ...
    if CT.solver_type == 0:
        CT_vars = 0
    elif CT.solver_type == 1:
        CT_vars = 1
    elif CT.solver_type == 2:
        CT_vars = 0
    else:
        print("Incorrect CT solver type")
        MyError('Incorrect CT solver type')
            
    T0 = np.zeros((4*G.n_cells)+(G.n_rows)+1+CT_vars)
...
def open_T(T,T_PF_in,TC_in,n_cells,n_rows,n_CPR,n_CP,n_passes,P_PF_in,PF_fluid,CT_solver_type):
    """
...
def equations(T,G,F,M,I,flag,CT):
    ...
    TH,TC,TW_H,TW_C,DV,mdot_vec,v,d3 = open_T(T,F.T_PF_in,F.TC_in,G.n_cells,G.n_rows,G.n_CPR,G.n_CP,G.n_passes,F.P_PF_in,F.PF_fluid,CT.solver_type)
    ...
    if CT.solver_type == 0: 
        Verror = Draft_equation(v,CT,F,TC_out_ave,dp_amb_total)*1e3
    ...
    if flag==0:
        ...
        return error, TH, TC, TW_H, TW_C, Q1, Q2, Q3, Q4, Q5, Q6, Q7, Q8, Q9, Alpha_i, PH, dp_amb, mdot_vec,P_headers, v, Alpha_o, dT, d3, Bu_c_array, Bu_j_array
