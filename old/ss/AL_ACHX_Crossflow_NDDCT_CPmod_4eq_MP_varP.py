...
def main(ACHX_inputs,mod=None,v=None,TC_in=None,T0i=[0],HX_L=None,fig_switch=0):
    ...
    if mod: # If the function exists, run the function that modifies one or more of the heat exchanger parameters. Useful for running a series of tests in which one parameter varies. 
        ...
        mod(M)
        mod(G)
    print("Nu Correlation: ", M.Nu_CorrelationH)
   ...
   # Creates initial solution values from inputs or interpolation function
    ...
    sol = sci.optimize.newton_krylov(lambda T:equations(T,G,F,M,I,0),T0,method='lgmres',f_tol=1e-6,callback=callback_function)
    T = sol
    ...
    error, TH_final, TC_final, TW_H_final, TW_C_final, Q1, Q2, Q3, Q4, Q5, Q6, Q7, Q8, Q9, alpha_i_final, PH, dp_amb, mdot_vec,P_headers, alpha_o_final, dT, Bu_c, Bu_j = equations(T,G,F,M,I,1)
    P_out = P_headers[-1]
    E_out = []
    if G.n_passes%2==0:
    ...
    return TC_out_ave, dp_amb_total, T, Q_PF, PF_dp, TH_final, dp_amb, alpha_i_final, TW_H_final, TW_C_final, Q3, PH, TC_final, alpha_o_final, dT, mdot_vec, G, Bu_c, Bu_j, T_out
...
def get_T0_iter(G,F,M,I):
    ...
    for i in range(G.n_rows):
        ...
        T0[2*G.n_cells:3*G.n_cells] =  F.T_PF_in - ((F.T_PF_in - F.TC_outlet_guess)*(((i+0.48)/G.n_rows)))
        T0[3*G.n_cells:4*G.n_cells] =  F.T_PF_in - ((F.T_PF_in - F.TC_outlet_guess)*(((i+0.95)/G.n_rows)))
    T0[4*G.n_cells:(4*G.n_cells)+G.n_rows] = 1000*F.mdot_PF/G.n_CP # Initial mass flow rate is total divided by all rows
    return T0
...
def open_T(T,T_PF_in,TC_in,n_cells,n_rows,n_CPR,n_CP,n_passes,P_PF_in,PF_fluid):
    ...
    mdot_vec[0:n_rows] = T[4*n_cells:(4*n_cells)+n_rows]/1000
...
#        Cp_f = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,P,Tf],[CPCP.iCpmass])
    ...
    if Correlation == 6:    # From ASPEN, for liquid being cooled
        mu_w = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,P,Tw],[CPCP.iviscosity])
        if F.PF_fluid == "CO2":
            Fp = 1
        else:
        ...
        if Re > 30000:
            ...
            # Nu = max(Nu_e3,Nu_t)
            #Beta = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,P,Tf],[CPCP.isobaric_expansion_coefficient])
            #Gr = Beta * 9.81 * (Dh**3) * (rho_b**2) * (Tb-Tw) / (mu_b**2)
    return Nu
...
def equations(T,G,F,M,I,flag):
    ...
    rho_PF_in = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,F.P_PF_in,F.T_PF_in],[CPCP.iDmass])
    ...
    for a in range(G.n_passes):
        for b in range(G.n_CP):
            ...
            if DV[row] == 1:
                # Below calculations determine dynamic pressure at inlet, for later calculation of transfer from dynamic to static pressure 
                rho_PF_b1 = GetFluidPropLow(EosPF,[CPCP.PT_INPUTS,P_headers[a],T_headers[a]],[CPCP.iDmass]) 
                U_PF1 = abs(mdot_vec[row] / (rho_PF_b1 * G.A_CS)) 
                PH_dyn1 = 0.5 * rho_PF_b1 * (U_PF1**2)
                ...
                for c in range(1,G.n_CPR+1):
                    ...
                    PH_dyn = 0.5 * rho_PF_b * (U_PF**2)
                    dPH_dyn = PH_dyn - PH_dyn1
                    PH_dyn1 = PH_dyn
                    ...
                    dp = (rho_PF_b * f_PF * (G.dx/G.ID) * (U_PF**2) / 2) # + dPH_dyn # calculate friction head loss (Darcy Weisbach formula)
                    PH[node] = PH[node-1] - dp
                if M.consider_bends == 1:
                    dp_PF_bend = (rho_PF_b * M.bend_loss_coefficient * (U_PF**2) / 2) # + dPH_dyn
    ...
    if M.solver_type == 1 or 2: # Header pressure for mdot or outlet temperature boundary condition [(n_passes)*(n_CP-1) equations]
        for d in range(G.n_passes): 
            ...
            for e in range(G.n_CP-1):
                ...
                error[4*G.n_cells + (G.n_passes-1)+ ((d*(G.n_CP-1))+e)] = np.sign(pc_delta_header_P)*(abs(pc_delta_header_P))*1e2 # Error for similar pressure between parallel streams at headers
    ...
    if M.solver_type == 1: # Total mass flow rate boundary condition
        error[(4*G.n_cells) + (G.n_passes-1)+ ((G.n_passes)*(G.n_CP-1))] = (F.mdot_PF - sum(mdot_vec[0:G.n_CP]))*1e3        
...
#        print(alpha_o)
        ...
        error[i] = (q1 + q2 - q3)#* (80/G.n_cells)
        error[G.n_cells + i] = (q4+q5-q6) #* (80/G.n_cells)
        error[2*G.n_cells + i] = (q3 - q4 + q7 - q8) #* (80/G.n_cells)
        error[3*G.n_cells + i] = (q9 - (min(q3,q4))) #* (80/G.n_cells)
    ...
    for j in range(G.n_passes-1):
        ...
        error[4*G.n_cells + j] = (mdot_header_in - mdot_header_out)*1e5
    ...
    if flag==0:
        ...
        return error, TH, TC, TW_H, TW_C, Q1, Q2, Q3, Q4, Q5, Q6, Q7, Q8, Q9, Alpha_i, PH, dp_amb, mdot_vec,P_headers, Alpha_o, dT, Bu_c_array, Bu_j_array
