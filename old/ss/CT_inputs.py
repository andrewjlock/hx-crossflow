...
# This is the main function that executes a result
    d3 = [0];v=[0]
    TC_out_ave, dp_amb_total, T1, Q_PF, PF_dp, TH_final, dp_amb, alpha_i_final, TW_H_final, TW_C_final, Q3, PH, TC_final, alpha_o_final, dT, mdot_vec, G, d3[0], v[0], Bu_c, Bu_j, T_out = CT_solver.main(ACHX_inputs,T0i = T0,fig_switch=0, mod=mod,verbosity=verbosity)
...
def ACHX_inputs(G,F,M,CT):    
    ...
    G.dx_i = 1.2001  #0.4 # (m) Initial length of pipe discretisation
    ...
    M.cooler_type = 0 # Switch for cooler type - (0) - forced convection, (1) - NDDCT
    ...
    F.PF_fluid  = 'CO2'       
    F.Amb_fluid  = 'Air'
    F.T_PF_in  = 354.55 #  CO2 inlet temperature (K)
    F.T_PF_out = 273.15+40 # CO2 temperature outlet (K) 
    F.P_PF_in  = 9*10**6 # CO2 inlet pressure (Pa)
    
    F.TC_in  = 273.15+30 # Air ground level temperature (K) 
    F.vC_in  = 1 # Initialisation air velocity (m/s) - solved during run
    F.P_amb_in  = 101325 # Atmospheric pressure (Pa)
    F.TC_outlet_guess = 273.15+55.1 # Air side outlet guess for initialisation (K)
    ...
    CT.d3 = 47.338# Diameter of cooling tower inlet for initialisation
    CT.mdot_PF_total = 344.4*0.643  # Total mass flow rate through the cooling tower
